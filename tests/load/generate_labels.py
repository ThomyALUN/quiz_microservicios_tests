...
endpoint = "http://localhost:8000/api/generate_labels"  # Ruta a evaluar
request_number = 30  # Número de usuarios concurrentes
test_type = "carga"  # Tipo de prueba: "estrés" o "carga"

def make_request(request):
    start_time = time.time()
    try:
        response = requests.post(endpoint, json={"n": random.randint(1, 3)})
...
print(f"Ruta evaluada: {endpoint}")
